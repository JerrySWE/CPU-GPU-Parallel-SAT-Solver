...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaMalloc(&device_x, vectorBytes);
    cudaMalloc(&device_y, vectorBytes);
    cudaMalloc(&device_result, vectorBytes);
    ...
    double executionDuration = kernelEndTime - kernelStartTime;
    printf("CUDA Execution Time: %.3f ms\n", 1000.f * executionDuration);
